...
import { z } from 'zod';
import { Sliders, Plus, Edit2, Trash2, Copy, AlertTriangle, ArrowUp, ArrowDown, GripVertical } from 'lucide-react';
...
export default function ModifiersPage() {
  ...
  const isEditable = hasPermission('modifiers.update') || hasPermission('modifiers.create');
  ...
  const modifierFormSchema = React.useMemo(() => {
    return z.object({
      ...
      message: tVal('maxSelect') || 'minSelect cannot exceed maxSelect',
      path: ['minSelect'],
    });
  }, [tVal]);
  ...
  const [editingGroup, setEditingGroup] = React.useState<ModifierGroup | null>(null);
  ...
  // Fetch list of modifier groups
  const { data, isLoading, error } = useQuery({
    queryKey: ['modifiers', { search, page, pageSize, sortBy, sortOrder }],
    queryFn: () =>
      apiClient.get<{ items: ModifierGroup[]; total: number }>(
        `/modifiers?search=${encodeURIComponent(search)}&page=${page}&pageSize=${pageSize}&sortBy=${sortBy}&sortOrder=${sortOrder}`
      ),
  });

  const modifierGroups = data?.items || [];
  ...
  const isRequiredVal = watch('isRequired');

  // Automatically update minSelect to 1 if required is toggled, and 0 if unchecked
  React.useEffect(() => {
    if (isRequiredVal) {
      setValue('minSelect', 1);
    } else {
      setValue('minSelect', 0);
    }
  }, [isRequiredVal, setValue]);

  // Determine if options have also been modified to check unsaved changes protection
  const isOptionsDirty = React.useMemo(() => {
    if (!editingGroup) {
      return localOptions.length > 1 || localOptions[0]?.name !== '' || localOptions[0]?.priceAdjustment !== 0;
    }
    ...
    for (let i = 0; i < localOptions.length; i++) {
      ...
      if (loc.id !== orig.id || loc.name !== origName || loc.priceAdjustment !== parseFloat(orig.priceAdjustment) || loc.displayOrder !== orig.displayOrder) {
        return true;
      }
    }
    return false;
  ...
  const createMutation = useMutation({
    mutationFn: (newGroup: any) => apiClient.post<ModifierGroup>('/modifiers', newGroup),
    onSuccess: (data) => {
      const name = data.translations.find((t) => t.locale === locale)?.name || 'Modifier Group';
      toastApp.success('create', `Modifier Group "${name}"`);
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, updates }: { id: string; updates: any }) =>
      apiClient.put<ModifierGroup>(`/modifiers/${id}`, updates),
    onSuccess: (data) => {
      const name = data.translations.find((t) => t.locale === locale)?.name || 'Modifier Group';
      toastApp.success('update', `Modifier Group "${name}"`);
      queryClient.invalidateQueries({ queryKey: ['modifiers'] });
      setHighlightedId(data.id);
      handleCloseDrawer();
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: ({ id }: { id: string; name: string }) => apiClient.delete(`/modifiers/${id}`),
    onSuccess: (data, variables) => {
      toastApp.success('delete', `Modifier Group "${variables.name}"`);
      setSelectedIds([]);
      queryClient.invalidateQueries({ queryKey: ['modifiers'] });
    },
  ...
  const duplicateMutation = useMutation({
    mutationFn: (id: string) => apiClient.post<ModifierGroup>(`/modifiers/${id}/duplicate`, {}),
    onSuccess: (data) => {
      const name = data.translations.find((t) => t.locale === locale)?.name || 'Modifier Group';
      toastApp.success('duplicate', `Modifier Group "${name}"`);
      queryClient.invalidateQueries({ queryKey: ['modifiers'] });
      setHighlightedId(data.id);
      setTimeout(() => {
        const el = document.getElementById(`row-${data.id}`);
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: async (ids: string[]) => {
      return promisePool(ids, (id) => apiClient.delete(`/modifiers/${id}`), 3);
    },
    onSuccess: (results) => {
      if (results.failed > 0) {
        toastApp.warning(`Deleted ${results.success} modifier groups. ${results.failed} deletions failed.`);
      } else {
        toastApp.success('delete', `${results.success} modifier groups`);
      }
      setSelectedIds([]);
      queryClient.invalidateQueries({ queryKey: ['modifiers'] });
    },
  ...
  const handleOpenCreateDrawer = () => {
    ...
    setLocalOptions([{ priceAdjustment: 0.00, displayOrder: 0, name: '' }]);
    setOptionsError(null);
    setIsDrawerOpen(true);
  };

  const handleOpenEditDrawer = (group: ModifierGroup) => {
  ...
  const handleAddOption = () => {
    setLocalOptions((prev) => [
      ...prev,
      { priceAdjustment: 0.00, displayOrder: prev.length, name: '' },
    ]);
  };
  ...
  const handleOptionChange = (index: number, field: 'name' | 'priceAdjustment', value: any) => {
    setLocalOptions((prev) =>
  ...
  const onSubmit = (values: ModifierFormValues) => {
    if (localOptions.length === 0) {
      setOptionsError(t('validation.optionsCount') || 'At least one modifier option is required.');
      return;
    }
    ...
    const hasNegativePrice = localOptions.some((opt) => opt.priceAdjustment < 0 || isNaN(opt.priceAdjustment));
    if (hasNegativePrice) {
      setOptionsError(t('validation.priceAdjustment') || 'Price adjustment must be a positive number.');
      return;
    }
  ...
  const handleDelete = (id: string, name: string) => {
    setConfirmDialog({
      isOpen: true,
      title: t('delete') || 'Delete Modifier Group',
      description: t('deleteConfirm', { name }) || `Are you sure you want to delete the modifier group "${name}"? This action cannot be undone.`,
      type: 'delete',
  ...
  const columns: ColumnDef<ModifierGroup>[] = [
    ...
    },
    {
      ...
      render: (item: ModifierGroup) => {
        const summary = item.options
          .map((opt) => {
            const optName = opt.translations.find((t) => t.locale === locale)?.name || opt.translations[0]?.name;
            const price = parseFloat(opt.priceAdjustment);
            return price > 0 ? `${optName} (+${price} EGP)` : optName;
          })
          .join(', ');
        return <span className="text-xs text-[var(--muted-foreground)] truncate max-w-sm block">{summary || '-'}</span>;
      },
      csvValue: (item: ModifierGroup) => {
        return item.options
          .map((opt) => {
            const optName = opt.translations.find((t) => t.locale === locale)?.name || opt.translations[0]?.name;
            const price = parseFloat(opt.priceAdjustment);
    ...
    },
    {
      ...
      render: (item: ModifierGroup) => (
        ...
        >
          {item.isRequired ? t('fields.isRequired') : tCommon('optional') || 'Optional'}
        </span>
      ),
      csvValue: (item: ModifierGroup) => item.isRequired ? 'Required' : 'Optional',
    },
    {
      header: `${t('fields.minSelect')} / ${t('fields.maxSelect')}`,
      render: (item: ModifierGroup) => (
        <span className="text-xs font-semibold">
          {item.minSelect} - {item.maxSelect}
        </span>
      ),
      csvValue: (item: ModifierGroup) => `${item.minSelect} - ${item.maxSelect}`,
    },
    {
      header: t('fields.displayOrder'),
      accessorKey: 'displayOrder',
      sortable: true,
      render: (item: ModifierGroup) => <span className="font-mono text-xs">{item.displayOrder}</span>,
    },
    {
      ...
      render: (item: ModifierGroup) => (
        <span
          className={`inline-flex items-center rounded-md border px-2 py-0.5 text-[10px] font-semibold transition-all ${
            item.isActive
      ...
      csvValue: (item: ModifierGroup) => item.isActive ? 'Active' : 'Inactive',
    },
    ...(isEditable
      ? [
          {
            ...
            render: (item: ModifierGroup) => {
              const tr = item.translations.find((t) => t.locale === locale) || item.translations[0];
              const name = tr?.name || 'Modifier Group';
              return (
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t('title')} description={t('description')} icon={Sliders} />

      {error ? (
        <div className="rounded-xl border border-danger-500/20 bg-danger-500/5 p-4 flex items-center gap-3">
          <AlertTriangle className="h-5 w-5 text-danger-500" />
          <p className="text-sm text-danger-500">
            {error.message || 'Error loading modifier templates. Check API connection.'}
          </p>
        </div>
      ) : (
        <DataTable<ModifierGroup>
          ...
          columns={columns}
          isLoading={isLoading}
          storageKey="modifiers"
          exportFilename="modifiers"
          ...
          bulkActions={[
            {
              ...
              onClick: (ids) => {
                setConfirmDialog({
                  isOpen: true,
                  title: t('delete') || 'Delete Modifier Groups',
                  description: t('deleteConfirmBulk', { count: ids.length }) || `Are you sure you want to delete ${ids.length} selected modifier groups?`,
                  type: 'delete',
      ...
      >
        ...
        {activeTab === 'details' ? (
          <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
            ...
            {/* Config: Required, minSelect, maxSelect */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 border-t border-[var(--border)]/40 pt-4">
              <div className="flex flex-col">
                <span className="text-xs font-semibold text-[var(--foreground)] mb-2">{t('fields.isRequired')}</span>
                <label className="inline-flex items-center cursor-pointer mt-1">
                  <input type="checkbox" className="sr-only peer" {...register('isRequired')} />
              ...
              <div>
                <label className="block text-xs font-semibold text-[var(--foreground)]" htmlFor="minSelect">
                  {t('fields.minSelect')}
                </label>
                <input
                  id="minSelect"
                  type="number"
                  placeholder={t('placeholders.minSelect')}
                  {...register('minSelect')}
                ...
                />
                {errors.minSelect && <p className="mt-1 text-xs text-danger-500">{errors.minSelect.message}</p>}
              </div>

              <div>
                <label className="block text-xs font-semibold text-[var(--foreground)]" htmlFor="maxSelect">
                  {t('fields.maxSelect')}
                </label>
                <input
                  id="maxSelect"
                  type="number"
                  placeholder={t('placeholders.maxSelect')}
                  {...register('maxSelect')}
                  className="mt-1 flex h-10 w-full rounded-lg border border-[var(--border)] bg-[var(--background)] px-3 py-2 text-sm text-[var(--foreground)] focus:border-brand-500 focus:outline-none focus:ring-1"
                />
              </div>
            ...
            <div className="border-t border-[var(--border)]/40 pt-4 space-y-3">
              ...
              <div className="hidden sm:flex items-center gap-2 px-2 py-1 text-[10px] font-bold text-[var(--muted-foreground)] border-b border-[var(--border)]/30 select-none">
                <div className="w-4 shrink-0" /> {/* grip space */}
                <div className="w-20 shrink-0 text-center">Harga (+EGP)</div>
                <div className="flex-1">Nama Opsi</div>
              ...
              <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
                {localOptions.map((opt, index) => (
                  <div
                    ...
                    className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 p-2 rounded-lg bg-[var(--accent)]/10 border border-[var(--border)] group/opt drag-transition"
                  >
                    <div className="flex items-center gap-2">
                      {/* Grip handler */}
                      ...
                      <div className="w-20 shrink-0">
                        <input
                          ...
                          className="flex h-9 w-full rounded-lg border border-[var(--border)] bg-[var(--background)] px-2 py-1 text-xs text-center focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 font-semibold"
                        />
                      </div>
                    </div>
          ...
          editingGroup && <AuditTimeline resource="ModifierGroup" resourceId={editingGroup.id} />
        )}
      </FormDrawer>
